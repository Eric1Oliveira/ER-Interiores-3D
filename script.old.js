...
// Mock Data
const mockCatalogs = [
  { id: 1, name: 'Design Elegância', description: 'Móveis planejados de alto padrão com design sofisticado e acabamento premium', specialty: 'Cozinhas Planejadas', experience: '15 anos', city: 'São Paulo', rating: 4.9, projects: 248, tags: ['Alto Padrão', 'Sob Medida'] },
  { id: 2, name: 'Madeira Premium', description: 'Criações exclusivas com materiais nobres e técnicas artesanais de excelência', specialty: 'Móveis Rústicos', experience: '20 anos', city: 'Curitiba', rating: 4.8, projects: 186, tags: ['Artesanal', 'Madeira Maciça'] },
  { id: 3, name: 'Contemporâneo', description: 'Linhas modernas e funcionais para espaços contemporâneos e dinâmicos', specialty: 'Home Office', experience: '8 anos', city: 'Rio de Janeiro', rating: 4.7, projects: 124, tags: ['Moderno', 'Funcional'] },
  { id: 4, name: 'Clássicos Sutis', description: 'Tradição e qualidade em cada detalhe com acabamentos impecáveis', specialty: 'Quartos Planejados', experience: '25 anos', city: 'Belo Horizonte', rating: 4.9, projects: 312, tags: ['Clássico', 'Premium'] },
  { id: 5, name: 'Inovação Móvel', description: 'Tecnologia e design integrados em soluções inteligentes de marcenaria', specialty: 'Closets', experience: '12 anos', city: 'Porto Alegre', rating: 4.6, projects: 98, tags: ['Tecnologia', 'Inovação'] },
  { id: 6, name: 'Sustentável Design', description: 'Materiais conscientes e design responsável para ambientes equilibrados', specialty: 'Móveis Externos', experience: '10 anos', city: 'Florianópolis', rating: 4.8, projects: 156, tags: ['Sustentável', 'Ecológico'] }
];

const mockUsers = [
  { id: 1, email: 'admin@marceneiroshub.com', password: 'admin123', name: 'Administrador', role: 'admin' },
  { id: 2, email: 'vendedor@marceneiroshub.com', password: 'vendedor123', name: 'João Vendedor', role: 'employee' },
  { id: 3, email: 'cliente@email.com', password: 'cliente123', name: 'Maria Cliente', role: 'client' }
];

const mockChats = [
  { id: 1, clientId: 3, clientName: 'Maria Cliente', catalogId: 1, catalogName: 'Móveis Elegance', status: 'active', lastMessage: 'Olá, gostaria de um orçamento', lastMessageTime: new Date(), unread: 2 },
  { id: 2, clientId: 4, clientName: 'José Silva', catalogId: 2, catalogName: 'Arte em Madeira', status: 'pending', lastMessage: 'Preciso de uma mesa de jantar', lastMessageTime: new Date(Date.now() - 3600000), unread: 1 }
];

const mockMessages = [
  { id: 1, chatId: 1, senderId: 3, senderName: 'Maria Cliente', senderRole: 'client', content: 'Olá, gostaria de um orçamento para uma cozinha planejada', timestamp: new Date(Date.now() - 7200000) },
  { id: 2, chatId: 1, senderId: 2, senderName: 'João Vendedor', senderRole: 'employee', content: 'Olá Maria! Claro, vi que você veio pelo catálogo Móveis Elegance. Poderia me contar mais sobre o espaço?', timestamp: new Date(Date.now() - 3600000) },
  { id: 3, chatId: 1, senderId: 3, senderName: 'Maria Cliente', senderRole: 'client', content: 'A cozinha tem aproximadamente 12m², formato em L', timestamp: new Date(Date.now() - 1800000) }
];

const mockProjects = [
  { id: 1, clientId: 3, clientName: 'Maria Cliente', catalogId: 1, catalogName: 'Móveis Elegance', title: 'Cozinha Planejada', status: 'in_progress', value: 15000, createdAt: new Date(Date.now() - 86400000 * 5) },
  { id: 2, clientId: 3, clientName: 'Maria Cliente', catalogId: 3, catalogName: 'Design Moderno', title: 'Home Office Completo', status: 'completed', value: 8500, createdAt: new Date(Date.now() - 86400000 * 30) }
];

// Initialize
state.catalogs = mockCatalogs;
...
function render() {
  ...
  switch(state.currentView) {
    ...
    case 'client-dashboard': app.innerHTML = renderHome(); break; // Client stays on main site
    ...
    case 'chat':
      if (state.userRole === 'client') {
        app.innerHTML = renderHome(); // Client uses lateral chat
  ...
  // Render floating chat button for clients
...
function openModal(type, data) {
  closeModal(true); // close any existing
  let content = '';
...
function closeModal(instant) {
  ...
  // Reset view if we navigated to login/register/catalog-detail
    ...
    <div class="p-8 md:p-10">
      ...
      <div class="bg-linen rounded-xl p-4 border border-sand/50">
        ...
        <div class="space-y-1 text-xs text-charcoal/40">
          <p><strong class="text-forest">Admin:</strong> admin@marceneiroshub.com / admin123</p>
          <p><strong class="text-forest">Vendedor:</strong> vendedor@marceneiroshub.com / vendedor123</p>
    ...
    <div>
      <div class="h-44 catalog-cover flex items-center justify-center rounded-t-[1.25rem]">
        <div class="text-center relative z-10">
          ...
          <p class="text-white/80 font-semibold text-lg">${catalog.name}</p>
          <p class="text-white/40 text-sm mt-1">${catalog.specialty}</p>
        </div>
      </div>
      <div class="p-8 md:p-10">
        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-5 mb-7">
          <div>
            <div class="flex items-center gap-4 mb-3">
              <h2 class="text-2xl font-bold text-forest">${catalog.name}</h2>
            ...
            <p class="text-charcoal/45 text-sm leading-relaxed">${catalog.description}</p>
            ${catalog.tags ? `
              <div class="flex gap-2 mt-3">
                ${catalog.tags.map(t => `<span class="text-[11px] font-semibold text-emerald2/70 bg-emerald2/[0.06] border border-emerald2/[0.1] px-3 py-1 rounded-lg">${t}</span>`).join('')}
              </div>
            ` : ''}
          </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-7">
          ${[
            ...
            { icon: 'briefcase', label: 'Projetos', value: catalog.projects + ' realizados', color: 'copper' }
          ].map(item => `
            <div class="bg-linen rounded-xl p-4 border border-sand/40">
              ...
              <p class="font-bold text-forest text-sm">${item.value}</p>
            </div>
          `).join('')}
        </div>
...
function renderClientProjectsModal() {
  ...
  const projects = mockProjects.filter(p => p.clientId === state.currentUser.id);
              ...
              <div class="flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-1">
                    <h4 class="font-semibold text-forest text-sm">${project.title}</h4>
                  ...
                  <p class="text-xs text-charcoal/40">Via catálogo: ${project.catalogName}</p>
            ...
            <button onclick="closeModal(true); openLateralChatById(${chat.id})" 
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between gap-2">
                  <h4 class="font-semibold text-forest text-sm truncate">${chat.catalogName}</h4>
                ...
                <p class="text-xs text-charcoal/40 truncate mt-0.5">${chat.lastMessage}</p>
              </div>
              ${chat.unread > 0 ? `
    ...
    <div class="p-8 md:p-10">
      ...
      <div class="space-y-5">
        <div>
          ...
          <input type="text" value="${state.currentUser.name}" class="input-modern" readonly>
        </div>
        <div>
          <label class="block text-xs font-semibold text-charcoal/50 mb-1.5 uppercase tracking-wide">Email</label>
          <input type="email" value="${state.currentUser.email}" class="input-modern" readonly>
        </div>
        <div class="bg-linen rounded-xl p-4 border border-sand/40">
    ...
    <header class="glass sticky top-0 z-50 shadow-soft border-b border-sand/50" style="backdrop-filter:blur(12px); background:rgba(250,250,248,0.92);">
      <div class="max-w-7xl mx-auto px-6 h-[72px] flex items-center justify-between">
        <div class="flex items-center gap-3 cursor-pointer group" onclick="navigate('home')">
          ...
          <div>
            <h1 class="text-base font-bold text-forest tracking-tight leading-tight">${state.siteTitle}</h1>
          ...
          <div class="flex items-center gap-2">
            ${state.currentUser ? `
              ${state.userRole === 'client' ? `
                <!-- Client: profile dropdown instead of dashboard button -->
                <div class="relative" id="profile-dropdown-container">
                  <button onclick="toggleProfileDropdown()" class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-linen border border-sand/60 hover:border-emerald2/30 transition cursor-pointer">
                    ...
                    <span class="text-sm text-forest font-medium hidden sm:inline">${state.currentUser.name}</span>
                  ...
                  <div id="profile-dropdown" class="profile-dropdown hidden absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-sand/60 overflow-hidden z-[60]">
                    <div class="p-4 border-b border-sand/40 bg-linen/50">
                      <p class="text-sm font-bold text-forest">${state.currentUser.name}</p>
                      <p class="text-[11px] text-charcoal/40">${state.currentUser.email}</p>
                    </div>
                    <div class="py-2">
                      <button onclick="openClientProjects()" class="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-linen transition text-left">
                        ...
                        ${mockProjects.filter(p => p.clientId === state.currentUser.id).length > 0 ? `
                          <span class="ml-auto text-[10px] font-bold bg-emerald2/10 text-emerald2 px-2 py-0.5 rounded-full">${mockProjects.filter(p => p.clientId === state.currentUser.id).length}</span>
                        ` : ''}
                      </button>
              ...
              ` : `
                <!-- Employee/Admin: keep dashboard button -->
                <div class="flex items-center gap-2">
            ...
            <div class="bg-white rounded-2xl overflow-hidden card-hover cursor-pointer shadow-soft border border-sand/60 hover:border-emerald2/25 group animate-slide-up delay-${(i % 3) + 1}" onclick="viewCatalog(${catalog.id})">
              ...
              <div class="p-6">
                <div class="flex items-center justify-between mb-2">
                  <h3 class="text-base font-bold text-forest">${catalog.name}</h3>
                  <div class="flex items-center gap-1 text-emerald2">
                    <i data-lucide="star" class="w-3.5 h-3.5 fill-current"></i>
                    <span class="text-sm font-bold">${catalog.rating}</span>
                  </div>
                </div>
                <p class="text-charcoal/40 text-sm mb-4 leading-relaxed line-clamp-2">${catalog.description}</p>
                <div class="flex flex-wrap gap-1.5 mb-4">
                  <span class="px-2.5 py-1 rounded-lg bg-forest/[0.05] text-forest text-[11px] font-semibold border border-forest/[0.08]">${catalog.specialty}</span>
                  <span class="px-2.5 py-1 rounded-lg bg-linen text-charcoal/50 text-[11px] font-medium border border-sand/50">${catalog.experience}</span>
                </div>
                <div class="flex items-center justify-between text-xs text-charcoal/35 font-medium pt-4 border-t border-sand/40">
                  ...
                  <span class="font-semibold text-forest">${catalog.projects} projetos</span>
                </div>
              </div>
            </div>
          `).join('')}
      ...
      <div class="max-w-7xl mx-auto">
        <div class="grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 class="text-3xl md:text-4xl font-extrabold text-forest mb-6 leading-tight">Por que escolher<br>${state.siteTitle}?</h2>
    ...
    <footer class="bg-charcoal text-white/50 py-14 px-6">
      <div class="max-w-7xl mx-auto">
        <div class="grid md:grid-cols-4 gap-10 mb-10">
          <div class="md:col-span-1">
            <div class="flex items-center gap-2 mb-4">
              ...
              <span class="text-sm font-bold text-white">${state.siteTitle}</span>
            </div>
        ...
        <div class="border-t border-white/[0.06] pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p class="text-xs text-white/25">&copy; 2024 ${state.siteTitle}. Todos os direitos reservados.</p>
...
function renderLogin() {
  return `
    ${renderHeader(false)}
    <div class="min-h-[calc(100vh-72px)] flex items-center justify-center px-6 py-14 bg-linen">
      <div class="w-full max-w-md animate-slide-up">
        <div class="bg-white rounded-2xl p-8 md:p-10 shadow-md border border-sand/50">
          <div class="text-center mb-8">
            <div class="w-14 h-14 mx-auto mb-4 rounded-2xl bg-forest/[0.06] flex items-center justify-center">
              <i data-lucide="lock" class="w-6 h-6 text-forest"></i>
            </div>
            <h2 class="text-2xl font-bold text-forest">Bem-vindo de volta</h2>
            <p class="text-charcoal/40 mt-2 text-sm">Entre na sua conta para continuar</p>
          ...
          <div id="login-error" class="hidden mb-4 p-3 rounded-xl bg-red-50 text-red-600 text-sm border border-red-100 text-center font-medium"></div>
          
          <form id="login-form" class="space-y-5">
            <div>
              <label class="block text-xs font-semibold text-charcoal/50 mb-1.5 uppercase tracking-wide">Email</label>
              <input type="email" id="login-email" required
                class="w-full px-4 py-3 rounded-xl border-2 border-sand/60 bg-white text-forest placeholder-charcoal/25 transition text-sm"
        ...
        <div class="mt-6 bg-white rounded-2xl p-6 border border-sand/50 shadow-soft">
          <p class="text-[10px] text-charcoal/30 font-semibold text-center mb-3 uppercase tracking-widest">Contas de demonstração</p>
          <div class="space-y-1.5 text-xs text-charcoal/40">
            <p><strong class="text-forest">Admin:</strong> admin@marceneiroshub.com / admin123</p>
            <p><strong class="text-forest">Vendedor:</strong> vendedor@marceneiroshub.com / vendedor123</p>
            <p><strong class="text-forest">Cliente:</strong> cliente@email.com / cliente123</p>
          </div>
...
function renderRegister() {
  return `
    ${renderHeader(false)}
    <div class="min-h-[calc(100vh-72px)] flex items-center justify-center px-6 py-14 bg-linen">
      <div class="bg-white rounded-2xl p-8 md:p-10 w-full max-w-md animate-slide-up shadow-md border border-sand/50">
        <div class="text-center mb-8">
          <div class="w-14 h-14 mx-auto mb-4 rounded-2xl bg-forest/[0.06] flex items-center justify-center">
            <i data-lucide="user-plus" class="w-6 h-6 text-forest"></i>
          </div>
        ...
        <div id="register-error" class="hidden mb-4 p-3 rounded-xl bg-red-50 text-red-600 text-sm border border-red-100 text-center font-medium"></div>
        <div id="register-success" class="hidden mb-4 p-3 rounded-xl bg-emerald-50 text-emerald-700 text-sm border border-emerald-100 text-center font-medium"></div>
...
function renderCatalogDetail() {
  ...
  return `
    ...
    <div class="max-w-5xl mx-auto px-6 py-10">
      <button onclick="navigate('home')" class="flex items-center gap-2 text-charcoal/40 hover:text-forest transition mb-8 text-sm font-medium">
        <i data-lucide="arrow-left" class="w-4 h-4"></i>
        Voltar aos catálogos
      </button>
      
      <div class="bg-white rounded-2xl overflow-hidden animate-slide-up shadow-md border border-sand/50">
        <div class="h-52 catalog-cover flex items-center justify-center">
          <div class="text-center relative z-10">
            <i data-lucide="layout-grid" class="w-14 h-14 text-white/40 mx-auto mb-3"></i>
            <p class="text-white/80 font-semibold text-lg">${catalog.name}</p>
          </div>
        </div>
        <div class="p-8 md:p-10">
          <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-8">
            <div>
              <div class="flex items-center gap-4 mb-3">
                <h1 class="text-2xl font-bold text-forest">${catalog.name}</h1>
                <div class="flex items-center gap-1 px-3 py-1 rounded-full bg-emerald2/[0.08] text-emerald2">
                  <i data-lucide="star" class="w-4 h-4 fill-current"></i>
                  <span class="font-bold text-sm">${catalog.rating}</span>
                </div>
              </div>
              <p class="text-charcoal/45 text-base leading-relaxed">${catalog.description}</p>
            </div>
            <button onclick="requestQuote(${catalog.id})" 
              class="btn-primary px-8 py-4 rounded-xl whitespace-nowrap inline-flex items-center gap-2 text-sm">
              <i data-lucide="message-circle" class="w-5 h-5"></i>
              Solicitar Orçamento
            </button>
          </div>
          ...
          <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-linen rounded-xl p-5">
              <div class="flex items-center gap-3 mb-2">
                <i data-lucide="award" class="w-5 h-5 text-emerald2"></i>
                <span class="text-charcoal/35 text-xs font-semibold uppercase tracking-wide">Especialidade</span>
              </div>
              <p class="font-bold text-forest text-sm">${catalog.specialty}</p>
            </div>
            <div class="bg-linen rounded-xl p-5">
              <div class="flex items-center gap-3 mb-2">
                <i data-lucide="clock" class="w-5 h-5 text-emerald2"></i>
                <span class="text-charcoal/35 text-xs font-semibold uppercase tracking-wide">Experiência</span>
              </div>
              <p class="font-bold text-forest text-sm">${catalog.experience}</p>
            </div>
            <div class="bg-linen rounded-xl p-5">
              <div class="flex items-center gap-3 mb-2">
                <i data-lucide="map-pin" class="w-5 h-5 text-forest"></i>
                <span class="text-charcoal/35 text-xs font-semibold uppercase tracking-wide">Localização</span>
              </div>
              <p class="font-bold text-forest text-sm">${catalog.city}</p>
            </div>
            <div class="bg-linen rounded-xl p-5">
              <div class="flex items-center gap-3 mb-2">
                <i data-lucide="briefcase" class="w-5 h-5 text-copper"></i>
                <span class="text-charcoal/35 text-xs font-semibold uppercase tracking-wide">Projetos</span>
              </div>
              <p class="font-bold text-forest text-sm">${catalog.projects} realizados</p>
            </div>
          </div>

          <div class="bg-linen rounded-xl p-6">
            <h3 class="font-bold text-forest mb-4 text-sm">Galeria de Projetos</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              ${[1,2,3,4].map(i => `
                <div class="aspect-square rounded-xl bg-gradient-to-br from-forest/10 to-emerald2/10 flex items-center justify-center">
                  <i data-lucide="${['armchair','sofa','door-open','archive'][i-1]}" class="w-10 h-10 text-forest/20"></i>
                </div>
              `).join('')}
            </div>
...
function renderClientDashboard() {
  const clientChats = state.chats.filter(c => c.clientId === state.currentUser.id);
  const clientProjects = state.projects.filter(p => p.clientId === state.currentUser.id);

  return `
    ${renderDashboardHeader('client')}
    <div class="flex h-[calc(100%-72px)]">
      ${renderDashboardSidebar('client')}
      <main class="flex-1 p-6 md:p-8 overflow-auto bg-linen scrollbar-thin">
        <div class="max-w-5xl mx-auto">
          <div class="mb-8">
            <h2 class="text-2xl font-bold text-forest">Olá, ${state.currentUser.name}!</h2>
            <p class="text-charcoal/40 text-sm mt-1">Acompanhe seus projetos e conversas</p>
          </div>
          
          <!-- Stats -->
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ${[
              { icon: 'message-circle', color: 'emerald2', value: clientChats.length, label: 'Conversas' },
              { icon: 'folder', color: 'forest', value: clientProjects.length, label: 'Projetos' },
              { icon: 'check-circle', color: 'emerald2', value: clientProjects.filter(p => p.status === 'completed').length, label: 'Concluídos' },
              { icon: 'clock', color: 'copper', value: clientProjects.filter(p => p.status === 'in_progress').length, label: 'Em andamento' }
            ].map(s => `
          ...
          <div class="bg-white rounded-2xl p-6 mb-6 shadow-soft border border-sand/50">
            <h3 class="font-bold text-forest mb-4 text-sm uppercase tracking-wide">Suas Conversas</h3>
            ${clientChats.length > 0 ? `
              <div class="space-y-2">
                ${clientChats.map(chat => `
                  <div onclick="openChat(${chat.id})" class="flex items-center justify-between p-4 rounded-xl hover:bg-linen transition cursor-pointer border border-sand/50">
                    <div class="flex items-center gap-4">
                      ...
                      <div>
                        <p class="font-semibold text-forest text-sm">${chat.catalogName}</p>
                        <p class="text-xs text-charcoal/35 mt-0.5">${chat.lastMessage}</p>
                      </div>
                    </div>
                    <div class="text-right">
                      <p class="text-[10px] text-charcoal/25">${formatDate(chat.lastMessageTime)}</p>
                      ${chat.unread > 0 ? `
                        <span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald2 text-white text-[10px] font-bold mt-1">${chat.unread}</span>
                      ` : ''}
                    </div>
                  </div>
                `).join('')}
            ...
            ` : `
              <div class="text-center py-10 text-charcoal/30">
                <i data-lucide="message-circle" class="w-12 h-12 mx-auto mb-3 opacity-30"></i>
                <p class="text-sm mb-3">Você ainda não tem conversas</p>
                <button onclick="navigate('home')" class="px-5 py-2 rounded-lg btn-primary text-xs">Explorar Catálogos</button>
              </div>
            `}
          </div>
          ...
          <div class="bg-white rounded-2xl p-6 shadow-soft border border-sand/50">
            <h3 class="font-bold text-forest mb-4 text-sm uppercase tracking-wide">Seus Projetos</h3>
            ${clientProjects.length > 0 ? `
              <div class="space-y-2">
                ${clientProjects.map(project => `
                  <div class="flex items-center justify-between p-4 rounded-xl border border-sand/50">
                    <div class="flex items-center gap-4">
                      <div class="w-10 h-10 rounded-full ${project.status === 'completed' ? 'bg-emerald2' : 'bg-forest'} flex items-center justify-center">
                        <i data-lucide="${project.status === 'completed' ? 'check' : 'loader'}" class="w-4 h-4 text-white"></i>
                      </div>
                      <div>
                        <p class="font-semibold text-forest text-sm">${project.title}</p>
                        <p class="text-xs text-charcoal/35 mt-0.5">${project.catalogName}</p>
                      </div>
                    </div>
                    <div class="text-right">
                      <p class="font-bold text-forest text-sm">${formatCurrency(project.value)}</p>
                      <span class="inline-block px-3 py-1 rounded-full text-[10px] font-semibold mt-1 ${project.status === 'completed' ? 'badge-active' : 'badge-pending'}">
            ...
            ` : `
              <div class="text-center py-10 text-charcoal/30">
                ...
                <p class="text-sm">Você ainda não tem projetos</p>
              </div>
            `}
          </div>
        </div>
              ...
              ${state.chats.map(chat => `
                <div onclick="openChat(${chat.id})" class="flex items-center justify-between p-4 rounded-xl hover:bg-linen transition cursor-pointer border border-sand/50">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <p class="font-semibold text-forest text-sm">${chat.clientName}</p>
                      <p class="text-xs text-charcoal/35 mt-0.5">
                        <span class="text-emerald2 font-medium">Via: ${chat.catalogName}</span> · ${chat.lastMessage}
                      </p>
                    </div>
                  </div>
                ...
                <tbody>
                  ${state.projects.map(project => `
                    <tr class="border-t border-sand/50">
                      <td class="py-4 text-forest">${project.clientName}</td>
                      <td class="py-4 text-forest font-medium">${project.title}</td>
                      <td class="py-4 text-charcoal/40">${project.catalogName}</td>
                      <td class="py-4 text-forest font-bold">${formatCurrency(project.value)}</td>
                      <td class="py-4">
              ...
              ${state.chats.map(chat => `
                <div onclick="openChat(${chat.id})" class="flex items-center justify-between p-4 rounded-xl hover:bg-linen transition cursor-pointer border border-sand/50 ${chat.status === 'active' ? 'border-l-2 border-l-emerald2' : 'border-l-2 border-l-copper'}">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <p class="font-semibold text-forest text-sm">${chat.clientName} — Vendedor</p>
                      <p class="text-xs text-charcoal/35">
                        Catálogo: <span class="text-emerald2 font-medium">${chat.catalogName}</span>
                      </p>
                      <p class="text-[10px] text-charcoal/20 mt-0.5">${chat.lastMessage}</p>
                    </div>
                  </div>
                  <div class="flex items-center gap-3">
                    ...
                    <span class="text-[10px] text-charcoal/25">${formatDate(chat.lastMessageTime)}</span>
                  </div>
                </div>
              `).join('')}
            </div>
              ...
              ${state.catalogs.map(catalog => `
                <div class="p-4 rounded-xl border border-sand/50 hover:border-sand transition">
                  <div class="flex items-center gap-3 mb-3">
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="font-semibold text-forest text-sm truncate">${catalog.name}</p>
                      <p class="text-[10px] text-charcoal/30">${catalog.city}</p>
                    </div>
                  </div>
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  ${state.users.map(user => `
                    <tr class="border-t border-sand/50">
                      <td class="py-4 text-forest font-medium">${user.name}</td>
                      <td class="py-4 text-charcoal/40">${user.email}</td>
                      <td class="py-4">
...
function renderDashboardHeader(role) {
  const roleLabels = {
    client: { label: 'Área do Cliente', icon: 'user' },
    employee: { label: 'Área do Vendedor', icon: 'briefcase' },
  ...
  return `
    <header class="bg-white h-[72px] px-6 flex items-center justify-between shadow-soft border-b border-sand/50">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="font-bold text-forest text-sm">${state.siteTitle}</h1>
      ...
      <div class="flex items-center gap-2">
        <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-linen border border-sand/40">
          ...
          <span class="text-xs text-forest font-medium hidden sm:inline">${state.currentUser.name}</span>
        </div>
...
function renderDashboardSidebar(role) {
  const menuItems = {
    client: [
      { icon: 'home', label: 'Início', action: 'client-dashboard' },
      { icon: 'message-circle', label: 'Conversas', action: 'client-chats' },
      { icon: 'folder', label: 'Projetos', action: 'client-projects' },
      { icon: 'grid-3x3', label: 'Catálogos', action: 'home' }
    ],
    employee: [
      { icon: 'home', label: 'Início', action: 'employee-dashboard' },
      { icon: 'message-circle', label: 'Chats', action: 'employee-chats' },
      { icon: 'folder', label: 'Projetos', action: 'employee-projects' },
      { icon: 'grid-3x3', label: 'Catálogos', action: 'home' }
    ],
    admin: [
      { icon: 'home', label: 'Dashboard', action: 'admin-dashboard' },
      { icon: 'eye', label: 'Monitor', action: 'admin-monitor' },
      { icon: 'grid-3x3', label: 'Catálogos', action: 'admin-catalogs' },
      { icon: 'users', label: 'Usuários', action: 'admin-users' },
      { icon: 'settings', label: 'Config', action: 'admin-settings' }
    ]
  };
...
function renderChatView() {
  ...
  if (!chat) return goToDashboard();

  const messages = mockMessages.filter(m => m.chatId === chat.id);
    ...
    <div class="h-full flex flex-col bg-linen">
      ...
      <header class="bg-white px-6 py-4 flex items-center justify-between shadow-soft border-b border-sand/50">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="font-semibold text-forest text-sm">${state.userRole === 'client' ? chat.catalogName : chat.clientName}</h2>
            <p class="text-[10px] text-charcoal/30">
              ${state.userRole === 'client' ? 'Conversa sobre orçamento' : 'Via catálogo: ' + chat.catalogName}
            </p>
          </div>
        </div>
        <div class="flex items-center gap-2">
      ...
      <div class="mx-4 mt-4 p-3 rounded-xl border border-emerald2/15 bg-emerald2/[0.03]">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-[10px] text-charcoal/35">${chat.catalogName}</p>
          </div>
        </div>
      </div>
...
// --- CLIENT PROFILE DROPDOWN & FUNCTIONS ---
...
function renderFloatingChatButton() {
  ...
  // If 1 chat, open it directly; if multiple, show messages list
  ...
  panel.innerHTML = `
    ...
    <div class="lateral-chat-panel">
      ...
      <div class="flex items-center gap-3 p-4 border-b border-sand/50 bg-white">
        ...
        <div class="flex-1 min-w-0">
          <h3 class="font-semibold text-forest text-sm truncate">${chat.catalogName}</h3>
      ...
      <div class="mx-3 mt-3 p-2.5 rounded-lg border border-emerald2/15 bg-emerald2/[0.03]">
        <div class="flex items-center gap-2">
          ...
          <p class="text-[11px] text-emerald2 font-medium">Catálogo: ${chat.catalogName}</p>
        </div>
      </div>
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (content && state.currentChat) {
        ...
        state.currentChat.lastMessage = content;
        state.currentChat.lastMessageTime = new Date();
        input.value = '';
        // Refresh the lateral chat panel
...
function goToDashboard() {
  ...
  switch(state.userRole) {
    ...
    case 'employee':
      ...
      break;
    case 'client':
      navigate('home'); // Client stays on main site
      break;
    default:
      navigate('home');
  }
...
function requestQuote(catalogId) {
  if (!state.currentUser) {
    closeModal(true);
    openModal('login');
    return;
  ...
  const newChat = {
    ...
    status: 'pending',
    lastMessage: 'Nova solicitação de orçamento',
...
function showAddCatalogModal() {
  // Simplified - in real app would show modal
  const name = 'Novo Catálogo ' + (state.catalogs.length + 1);
  const newCatalog = {
    id: state.catalogs.length + 1,
    name,
